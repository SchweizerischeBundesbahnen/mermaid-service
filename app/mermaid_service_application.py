...
from app import mermaid_controller  # type: ignore


def setup_logging() -> Path:
    """
    Configure logging for the Mermaid service with both file and console output.

    The function:
    - Sets log level from LOG_LEVEL environment variable (defaults to INFO)
    - Creates timestamped log files in /opt/mermaid/logs directory
    ...
    log_dir = Path(os.getenv("LOG_DIR", "/opt/mermaid/logs"))
    log_dir.mkdir(parents=True, exist_ok=True)

    # Create log filename with timestamp
    current_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    log_file = log_dir / f"mermaid-service_{current_time}.log"
...
def main() -> None:
    """
    Main entry point for the Mermaid service.

    Parses command line arguments, initializes logging, and starts the server.
    The service port can be specified via command line argument (defaults to 9084).
    """
    parser = argparse.ArgumentParser(description="Mermaid service")
    parser.add_argument("--port", default=9084, type=int, required=False, help="Service port")
    args = parser.parse_args()

    setup_logging()
    logging.info("Mermaid service listening port: " + str(args.port))

    mermaid_controller.start_server(args.port)
