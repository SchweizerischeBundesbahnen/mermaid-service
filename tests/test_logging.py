...
from app.mermaid_service_application import setup_logging
...
def wait_for_log_file(log_dir: Path, timeout: float = 1.0) -> Path:
    ...
    while time.time() - start_time < timeout:
        try:
            log_file = next(log_dir.glob("mermaid-service_*.log"))
...
def test_log_file_creation(log_dir):
    ...
    assert log_file.name.startswith("mermaid-service_")
